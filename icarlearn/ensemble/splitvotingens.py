...
class SplittingVotingEnsemble(BaseEstimator, ClassifierMixin):
    """
    Splitting And Voting Ensemble using a general Classifier set.
    ...
    :param verbose: If True, prints progress messages.
    :type verbose: bool
    :param split_seed: Seed for random splitting of dataset.
    :type split_seed: int
    """
    def __init__(self, clf, n_voters:int=-1, voting:str='soft', n_jobs:int=-1, verbose:bool=False, split_seed: int | None = 42):
        # intialize ensemble ov voters
        self.clf = clf
        assert is_classifier(clf), "clf must be a valid classifier object" 
        self.voting = voting
...
class sveLDA(SplittingVotingEnsemble):
    def __init__(self, n_voters:int=-1, voting:str='soft', n_jobs:int=-1, verbose:bool=False, split_seed:int=42, **kw):
        clf = LinearDiscriminantAnalysis(**kw)
        super().__init__(clf, n_voters=n_voters, voting=voting, n_jobs=n_jobs, 
                                                  verbose=verbose, split_seed=split_seed)
        for k,v in kw.items():
            setattr( self, k, v )
...
class sveLGBM(SplittingVotingEnsemble):
    def __init__(self, n_voters:int=-1, voting:str='soft', n_jobs:int=-1, verbose:bool=False, split_seed:int=42, **kw):
        clf = LGBMClassifier(verbose=-1, **kw)
        super().__init__(clf, n_voters=n_voters, voting=voting, n_jobs=n_jobs, 
                                                  verbose=verbose, split_seed=split_seed)
        for k,v in kw.items():
            setattr( self, k, v )
